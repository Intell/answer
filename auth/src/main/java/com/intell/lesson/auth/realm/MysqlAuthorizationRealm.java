...
import com.intell.lesson.auth.enums.UserStatus;
...
public class MysqlAuthorizationRealm extends AuthorizationRealm   {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
        ...
        if (user != null) {
            ...
            if (StringUtils.isNotEmpty(user.getHost()) &&user.getHost().equals(token.getHost())) {
//                ipConfirmed = authService.confirmDifferentIpLogin(token.getUsername(), token.getHost());
            }
            if (!ipConfirmed) {
